...
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, ResolveRecordResponse, HelloResponse};
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::Hello { name } => hello(name),
        QueryMsg::ResolveRecord { name } => query_resolver(deps, env, name),
        QueryMsg::Config {} => to_binary(&config_read(deps.storage).load()?),
    }
}

fn hello(name: String) -> StdResult<Binary> {
    let mut message_str: String = "Hello ".to_owned();
    message_str.push_str(&name);
    let message = Some(String::from(&message_str));
    let resp = HelloResponse { message };
    to_binary(&resp)
}
